...
casper
    .start("https://www.raw2k.co.uk/vehicles")
    .then(function() {
        qs.log("--");
...
function gatherAllAuctionPages() {
    this.then(function() {
        ...
        casper.waitForSelector('[class="coverlink"]', function() {
            addLinksToScrapeData.call(this);
            search.pages = this.evaluate(function() {
                var pages = [];
                Tpages = document
                    .querySelector('[for="page-number"]')
                    .innerText.trim()
                    .split(" ")[1];
                var pageCnt = parseInt(Tpages);

                for (var i = 1; i < pageCnt; i++) {
                    pages.push('[class="page-button next-page"]');
                }
                return pages;
            });
...
function gatherSearchResultLinks() {
    this.then(function() {
        ...
        if (search.pages[search.currentPage]) {
            ...
            var clickElem = search.pages[search.currentPage];
            this.then(function() {
                this.evaluate(function(clickElem) {
                    document.querySelector(clickElem).click();
                }, clickElem);
            });
            ...
            this.then(function() {
                ...
                this.then(function() {
                    this.waitForSelector('[class="coverlink"]', afterWait);
                });
            });
        }
    });
...
function getLinks() {
    ...
    var elem = document.querySelectorAll('[class="coverlink"]');
...
function spiderDetailsPage() {
    ...
    this.then(function() {
        if (scrapeData.links[scrapeData.currentData]) {
            ...
            this.then(function() {
                this.waitForSelector('[class="vehicle-image"]', afterWait);
            });
        } else {
            qs.log(
                "Total lots found: " +
...
function parse(lotData) {
    ...
    try {
        ...
        if (lotname) {
            ...
            if (lotnameVal[lotnameVal.length - 1].indexOf("cc") !== -1)
                lot["engine_size"] = lotnameVal[lotnameVal.length - 1]
                    .trim()
                    .split(" ")[0]
                    .trim();
        }
        ...
        try {
            lot["fuel"] = document
                .querySelector('[itemprop="fuelType"]')
                .innerText.trim();
        } catch (err) {}

        try {
            lot["gearbox"] = document
                .querySelector('[itemprop="vehicleTransmission"]')
                .innerText.trim();
        } catch (err) {}

        try {
            lot["registration"] = __utils__
                .getElementByXPath(
                    "/html/body/div[1]/div/div[3]/div[1]/div[2]/div/div[1]/div[3]/div/div[4]"
                )
                .innerText.trim();
        } catch (err) {}
        ...
        var auction_date =
            __utils__
                .getElementByXPath(
                    "/html/body/div[1]/div/div[3]/div[2]/div[1]/div[4]/div[1]/p"
                )
                .innerText.trim()
                .split("\n")[1] +
            " - " +
            __utils__
                .getElementByXPath(
                    "/html/body/div[1]/div/div[3]/div[2]/div[1]/div[4]/div[2]/p"
                )
                .innerText.trim()
                .split("\n")[1];

        if (auction_date.split("-").length > 1)
            lot["auction_date"] = auction_date.split("-")[1].trim();
        else if (auction_date.split("-").length == 1)
            lot["auction_date"] = auction_date.trim();

        try {
            lot["mileage"] = document
                .querySelector('[itemprop="mileageFromOdometer"]')
                .innerText.trim();
        } catch (err) {}

        try {
            lot["description"] = document.querySelector(
                ".maincolumn div:nth-child(3) div"
            ).innerText;
        } catch (err) {}
        ...
        lot["images"] = [].slice
            .call(
                document.querySelectorAll(
                    'a[href*="www.raw2k.co.uk/storage/uploads/"]'
                )
            )
            .map(function(a) {
                var src = a.href;
