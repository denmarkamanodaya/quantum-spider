...
function gatherAllAuctionLinks() {
    this.then(function() {
        ...
        this.waitForSelector(
			...
			//return elem[0].querySelector('h2[class*="name"]').innerText.trim();
                    ...
                    for (var x = 0; x < elem.length; x++) {
                        ...
                        if (auction_name.toLowerCase().indexOf("prees heath") != -1) {
                            auction_urls_temp.push({
                               // url: elem[x].querySelector('[href*="/vehicle-search/?search"]').href.trim()
				url: elem[x].querySelector('li[class*="cat"] a').href.trim()
                            });
                        }
                    }
