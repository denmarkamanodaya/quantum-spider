...
casper
    .start(
        "https://auctions.nsaa.co.uk/vehicle-list/?x=0&type=9&memr=2&make=&trns=0&fuel=0&catc=0&dist=0&sort=0&srch="
    )
    .then(function() {
        qs.log("--");
    ...
    this.then(function() {
        ...
        search.pages = casper.evaluate(function() {
            ...
            for (var i = 1; i <= total; i++) {
                pages.push(
                    "https://auctions.nsaa.co.uk/vehicle-list/?x=0&type=9&memr=2&make=&trns=0&fuel=0&catc=0&dist=0&sort=0&srch=&page=" +
                        i
                );
            }
...
function linkSpiderEventListeners() {
    ...
    casper.on("resource.requested", function(requestData, request) {
        if (!(requestData.url.indexOf("auctions.nsaa.co.uk") > -1)) {
            request.abort();
        }
    });
}
