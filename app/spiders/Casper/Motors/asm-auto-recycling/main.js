...
casper
	...
	"https://auctions.asm-autos.co.uk/vehicle-list/?type=0"
    )
    .then(function() {
        qs.log("--");
...
function gatherAllAuctionPages() {
    this.then(function() {
        ...
        search.pages = casper.evaluate(function() {
            var pages = [];
            var total = document
                .querySelectorAll('[class="vlo_results_info_span"]')[1]
                //.innerText.trim();
		.textContent.trim();
            ...
            for (var i = 1; i <= total; i++) {
                pages.push("https://auctions.asm-autos.co.uk/vehicle-list/?type=9&make=&trns=0&fuel=0&catc=0&memr=32&site=&dist=0&sort=0&srch=&page=" + i);
            }
            return pages;
        });
...
function parse(lotData) {
	...
	*/
	// Format auction_date
	var date_array = document.querySelector('[class="closing_details"]').innerText.trim().split(" ");
	...
	var new_month_int = ("0" + monthNameToNum(date_array[2])).slice(-2);
	var new_date_int = ("0" + date_array[1].replace(/\D/g,'')).slice(-2);
	var new_date = year + "-" + new_month_int + "-" + new_date_int;
	
	lot['auction_date'] = new_date;
