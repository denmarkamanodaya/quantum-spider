...
casper
    .start("https://www.pistonheads.com/classifieds?Category=used-cars&TradePrivateFilter=Private")
    .then(function() 
    {
    ...
    this.then(function() 
    {
        ...
        this.waitForSelector(
            ...
            function() 
            {
                ...
                {
                    var auction_urls            = [];
                    var element                 = document.querySelectorAll('ul.pages > li');                    
                    var pagination_last_count   = element[8].innerText.trim();
                    // var pagination_last_count	= 1;

                    for (var x = 1; x <= pagination_last_count; x++) 
                    {
                        var auction_url     = "https://www.pistonheads.com/classifieds?Category=used-cars&Page=" + x + "&TradePrivateFilter=Private";
    ...
    this.then(function() 
    {
        if (auctionIdx < auction_urls.length && auction_urls[auctionIdx]) 
        {
            ...
            this.then(function() 
            {
                ...
                this.then(function() 
                {
                    this.waitForSelector('div.listing-headline a', afterWait);
                });
            });
        }
    });
...
{   
    var links       = [];
    var element     = document.querySelectorAll('div.listing-headline a');

    // element.lenght
    for (var i = 0; i < element.length; i++) 
    {
        var lotname  = element[i].querySelector('h3').innerText.trim();
...
function parse(lotData) 
{
	...
	lot["breadcrumbs"]  = lot["manufacturer"] + " " + lot["model"] + " " + lot["year"];
	lot["lotname"]	    = document.querySelector("span.enquiryName").innerText.trim() + " " + document.querySelector("span.enquiryModel").innerText.trim();
	lot["name"]	    = lot["breadcrumbs"] + " " + lot["lotname"];
...
function linkSpiderEventListeners() 
{
    ...
    casper.on("resource.requested", function(requestData, request) 
    {
        if (!(requestData.url.indexOf("pistonheads") > -1)) 
        {
            request.abort();
        }
    });
