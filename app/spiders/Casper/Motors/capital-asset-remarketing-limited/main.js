...
casper
    .start("https://www.i-bidder.com/en-gb/auction-catalogues/car")
    .then(function() {
        qs.log("--");
...
function gatherAllCatalogueLinks() {
    this.then(function() {
        ...
        auction_urls = this.evaluate(function() {
            //GETTING CATALOGUE URLS FROM HOMEPAGE
            return Array.prototype.slice
                .call(document.querySelectorAll("#results .button.secondary"))
                .map(function(a) {
                    return a.href;
                });
        });
    ...
    this.then(function() {
        if (auctionIdx < auction_urls.length && auction_urls[auctionIdx]) {
            ...
            this.then(function() {
                ...
                this.then(function() {
                    this.waitForSelector('[class="lot-single"]', afterWait);
                });
            });
        }
    });
...
function getLinks() {
    var auctionDate = document
        .querySelector(".first-sale-date")
        .textContent.trim()
        .split("from")[1]
        .trim();
    var links = Array.prototype.slice
        .call(document.querySelectorAll(".lot-listing-results article h1 a"))
        .map(function(a) {
            return {
                url: a.href,
