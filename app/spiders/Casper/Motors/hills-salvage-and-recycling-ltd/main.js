...
casper
    .start(
        "https://auctions.hills-motors.co.uk/vehicle-list/?type=9&make=&trns=0&fuel=0&catc=0&memr=5&site=&dist=0&sort=0&srch="
    )
    .then(function() {
        qs.log("--");
    ...
    this.then(function() {
        ...
        search.pages = casper.evaluate(function() {
            ...
            for (var i = 1; i <= total; i++) {
                pages.push(
                    "https://auctions.hills-motors.co.uk/vehicle-list/?type=9&memr=5&make=&trns=0&fuel=0&catc=0&dist=0&sort=0&srch=&page=" +
                        i
                );
            }
...
function linkSpiderEventListeners() {
    ...
    casper.on("resource.requested", function(requestData, request) {
        if (!(requestData.url.indexOf("auctions.hills-motors.co.uk") > -1)) {
            request.abort();
        }
    });
}
