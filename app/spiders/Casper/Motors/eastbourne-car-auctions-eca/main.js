...
var baseURL = "http://www.ebca.co.uk/";
var baseApiURL =
    "http://online.ebca.co.uk/Stock/LoadStock?make=&model=&fuzzy=&bodyTypeId=0&page=1&pageSize=1000&sortby=l_ot&fuel=&transmission=&vendor=&buyItNow=False&catalogue=false&salecode=";
    ...
    for (var l = 0; l < apiData.StockItems.length; l++) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (key == "VehicleReferenceNumber") {
                key = "lot_url";
                val = "http://online.ebca.co.uk/stock/details?ref=" + val;
            }
            // if (key == "StockImage") {
            //     val = ["http://online.ebca.co.uk/image.ashx?path=" + val];
            // }
            if (key != "" && val != "" && val != null) {
		...
		//var dirty_year = ("20" + date_dirty[2].slice(-2));
                var clean_date = "20" + date_dirty[2] +  "-" + date_dirty[1] + "-" + date_dirty[0];;
...
function gatherDetails(url, lotData) {
    this.then(function() {
        ...
        this.thenOpen(finalUrl);
        lotData["images"] = this.evaluate(parse, lotData);
        // this.waitForSelector('img.imageThumb',
        //     function()
        //     { 
        //         lotData["images"] = this.evaluate(parse, lotData);
        //     }
...
{
    var X = [].slice.call(document.querySelectorAll('[class*="imageThumb"]'))
        .map(function(img) 
        {
            return "http://online.ebca.co.uk" + img.dataset.large;
